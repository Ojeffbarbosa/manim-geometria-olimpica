...
Se triângulos equiláteros são construídos sobre os lados de qualquer triângulo, então os centros desses 
triângulos equiláteros formam um triângulo equilátero.


Elementos da construção:
- ABC: triângulo original
- D, E, F: vértices externos dos triângulos equiláteros construídos sobre AB, BC e CA
- P, Q, R: centros dos triângulos equiláteros ABD, BCE e CAF
...
class NapoleonTheorem(Scene):
    def construct(self):
        ...
        def get_P():  # Centro do triângulo sobre AB
            return get_centro(get_vert_A(), get_vert_B(), get_vert_C())
        
        def get_Q():  # Centro do triângulo sobre BC
            return get_centro(get_vert_B(), get_vert_C(), get_vert_A())
        
        def get_R():  # Centro do triângulo sobre CA
        ...
        # Triângulo equilátero ABD (sobre o lado AB - amarelo)
        ...
        # Triângulo equilátero BCE (sobre o lado BC - azul)
        ...
        # Triângulo equilátero CAF (sobre o lado CA - verde)
        ...
        pt_A = always_redraw(lambda: Dot(get_vert_A(), color=COR_TXT, radius=0.06))
        pt_B = always_redraw(lambda: Dot(get_vert_B(), color=COR_TXT, radius=0.06))
        pt_C = always_redraw(lambda: Dot(get_vert_C(), color=COR_TXT, radius=0.06))
        ...
        self.play(FadeIn(pt_A), FadeIn(pt_B), FadeIn(pt_C))
        self.add(rot_A, rot_B, rot_C)
        self.play(Write(rot_A), Write(rot_B), Write(rot_C))
        self.wait()
        
        # --- ETAPA 2: CONSTRUIR OS TRIÂNGULOS EQUILÁTEROS EXTERNOS 
        self.play(Create(tri_ABD))
        self.wait(0.3)
        self.play(Create(tri_BCE))
        self.wait(0.3)
        self.play(Create(tri_CAF))
        self.wait()
